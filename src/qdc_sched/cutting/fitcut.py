...
class FitCutCutStrategy(CutStrategy):
    ...
    def _score_plan(self, plan: PartitionPlan, constraints: CutConstraints, context: Dict[str, Any]) -> float:
        ...
        qpu_candidates = (context or {}).get("qpu_candidates", None)
        ...
        penalty += float(self.cfg.recon_base_s) + float(self.cfg.recon_per_exec_s) * sampling
    ...
    def analyze(self, circuit: QuantumCircuit, constraints: CutConstraints, context: Dict[str, Any]) -> CutAnalysis:
        ...
        targets: List[Optional[int]] = [int(constraints.target_labels)] if constraints.target_labels is not None else list(range(2, max(2, cap) + 1))
        ...
        if best_plan is None:
            if os.getenv("QDC_C_FALLBACK_DUMMY", "0") == "1":
                try:
                    ...
                    ctx_max_local = 0
                if ctx_max_local > 0:
                    subcircs = _dummy_partition(circuit, ctx_max_local, int(getattr(last_cc, "target_labels", 2) or 2))
                    if subcircs:
                        return PartitionPlan(
                            kind="DUMMY_PARTITION",
                            subcircuits=subcircs,
                            reconstruction={"meta": {"fallback": True, "reason": "fitcut_failed", "max_local_qubits": ctx_max_local}},
                            est_executions=1,
                            k_wire=0,
                            k_gate=0,
                        )
