"""run_suite.py — Main DQC Scheduler experiment suite (E1–E16 equivalent).

Runs all scheduler experiment suites across one or more seeds and writes
per-run output directories containing records.csv, events.csv,
job_manifest.csv, and run_meta.json.
...
Usage
-----
    python run_suite.py --suite all --seeds 2026 --outdir results/experiment_suite
    python run_suite.py --suite paper_main --seeds 2026,2027,2028 --outdir results/paper_ci
    python run_suite.py --suite all --seeds 2026 --fast   # reduced job counts

After the run completes, aggregate results with:
...
def main() -> None:
    ...
    ap.add_argument(
        "--seeds", default="2026",
        help="Comma-separated random seeds (default: 2026)",
    )
    ap.add_argument(
        "--outdir", default="results/experiment_suite",
        help="Root output directory; one subdir is created per run",
    )
    ap.add_argument(
        "--fast", action="store_true",
        help="Reduce job counts for a quick smoke test",
    )
    args = ap.parse_args()

    if args.fast:
        # Patch workload job counts down for smoke test
        for wp in DEFAULT_WORKLOADS.values():
            object.__setattr__(wp, "n_jobs", min(wp.n_jobs, 8))
    ...
    print(f"[run_suite]  suite={args.suite}  seeds={seeds}  n_runs={len(runs)}")
    print(f"[run_suite]  outdir={out_root}  fast={args.fast}")

    manifest: List[Dict[str, Any]] = []
    t0 = time.perf_counter()
    for i, spec in enumerate(runs, 1):
        print(f"\n[{i}/{len(runs)}] {spec.run_name}")
    ...
    manifest_path.write_text(json.dumps(
        {"runs": manifest, "elapsed_wall_s": elapsed}, indent=2
    ))
    print(f"\n[run_suite]  {len(runs)} runs completed in {elapsed:.1f}s")
    print(f"[run_suite]  manifest -> {manifest_path}")
    print(f"\nNext step: python analyze_experiment_suite.py {out_root}")
